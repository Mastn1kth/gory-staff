...
- Пароль возвращается только в `new_credentials` результата текущего запуска
- `password_plain` не заполняется для iiko-sync, чтобы не хранить временный пароль в базе
...
2. Откройте раздел `Персонал`
3. Нажмите `Синхронизировать из iiko`
4. Если появились новые сотрудники, сразу сохраните показанные временные доступы и передайте их лично
...
- Временные пароли генерируются случайно (12 символов)
- Не сохраняются в `password_plain` при iiko-синхронизации
- Показываются только в `new_credentials` ответа на запуск синхронизации
